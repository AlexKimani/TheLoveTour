...
class CI_Session {
	...
	public function __construct($params = array ()) {
		...
		foreach (array ( 'sess_encrypt_cookie', 'sess_use_database', 'sess_table_name', 'sess_expiration',
				         ...
				         'cookie_domain', 'cookie_secure', 'sess_time_to_update', 'time_reference', 'cookie_prefix',
	...
	function _get_time() {
		if (strtolower($this->time_reference) == 'gmt') {
			$now = time();
			$time = mktime(gmdate("H", $now), gmdate("i", $now), gmdate("s", $now), gmdate("m", $now), gmdate("d", $now),
					gmdate("Y", $now));
		} else {
			$time = time();
		}
